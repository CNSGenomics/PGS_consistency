...
predictor.list = read.csv("Tables/SupTable2_Predictors_20250610.csv")
predictor.list = predictor.list[which(predictor.list$selected == "TRUE" & predictor.list$Predictor != "UKB_PPP_2022_IL12A_IL12B"),]
...
Most traits used 7.3M LD reference, but for those could not cover 70% of the SNPs, we switched to HapMap3 LD reference. 

```{r}
table(predictor.list$reference.version)
```

Here is a number of binary traits and quantitative traits.

```{r}
table(predictor.list$QorB)
```

There are 

`r length(traits_binary )` binary traits and   
`r length(predictor.list[which(predictor.list$QorB == "Quantitative" ),"Predictor"])` quantitative traits which includes


>`r length(traits_quanti )` collected quantitative traits     
>`r length(traits_35bm   )` biomarker traits from uk biobank study  
>`r length(traits_ppp    )` proteomics traits from uk biobank study    
>`r length(traits_fa    )` fatty acid from uk biobank study  
